...
use codex_utils_pty::OutputSink;
use codex_utils_pty::ProcessHandle;
use codex_utils_pty::SpawnedStreamingProcess;
...
struct RunCommandParams {
    ...
    spawned: SpawnedStreamingProcess,
...
impl CommandExecManager {
    pub(crate) async fn start(
        ...
        let stdin_mode = if stream_stdin {
            PipeStdinMode::Piped
        } else {
            PipeStdinMode::Null
        };
        ...
        let spawned = if tty {
            codex_utils_pty::pty::spawn_streaming_process(
                program,
                args,
                cwd.as_path(),
                &env,
            ...
            codex_utils_pty::pipe::spawn_streaming_process(
                program,
                args,
                cwd.as_path(),
                &env,
...
async fn run_command(params: RunCommandParams) {
    ...
    let SpawnedStreamingProcess { session, exit_rx } = spawned;
    tokio::pin!(exit_rx);
